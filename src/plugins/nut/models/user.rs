...
impl Item {
    pub fn available(&self) -> Result<()> {
        if self.deleted_at.is_some() {
            return Err(Error::Http(StatusCode::GONE, None));
        }
        if self.locked_at.is_some() {
            return Err(Error::Http(StatusCode::LOCKED, None));
        }
    ...
    pub fn auth<E: Password>(&self, password: &str) -> Result<()> {
        ...
        Err(Error::Http(StatusCode::UNAUTHORIZED, None))
    }
}
...
impl Dao for Connection {
    ...
    fn by_nickname(&self, nickname: &str) -> Result<Item> {
        let it = users::dsl::users
            .filter(users::dsl::nickname.eq(nickname.trim()))
            .first(self)?;
        Ok(it)
    }
