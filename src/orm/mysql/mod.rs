...
impl Dao for Connection {
    fn check(&self) -> Result<()> {
        let rst = sql_query("SELECT table_name AS name FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'schema_migrations'").load::<Table>(self)?;
        if rst.is_empty() {
