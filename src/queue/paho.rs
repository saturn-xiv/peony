...
impl Config {
    ...
    fn name() -> Result<String> {
        Ok(format!("{}-{}", hostname()?, Pid::this()))
    }

    pub fn consume<F>(&self, topics: Vec<String>, f: &F) -> Result<()>
    where
        F: Fn(&str, &str, &[u8]) -> Result<()>,
    {
                    ...
                    ConnectOptionsBuilder::new()
                        ...
                        .will_message(
                            MessageBuilder::new()
                                ...
                                .payload(flexbuffers::to_vec(&WillMessage { id: Self::name()? })?)
                                .finalize(),
                        )
                        .finalize(),
                )?;
                if let Some(rsp) = rsp.connect_response() {
                    ...
                    if !rsp.session_present {
                        for t in topics.iter() {
                            cli.subscribe_with_options(t, Self::QOS, None, None)?;
                        }
                    }
                }
            }
